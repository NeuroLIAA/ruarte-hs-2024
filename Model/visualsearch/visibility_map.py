...
class VisibilityMap:
    def __init__(self, image_size, grid, sigma, fovea_exponent = 2, peripheral_exponent = 1):
        self.visibility_map = self.create(image_size, grid, sigma)**peripheral_exponent
        self.fovea_map = self.create(image_size, grid, sigma)**fovea_exponent
        self.visibility_map_normalized = self.convert_to_unit_interval(self.visibility_map) #Without normalization here as it isn't needed
        self.fovea_map_normalized = self.convert_to_unit_interval(self.fovea_map)
        self.visibility_map *= np.max(self.fovea_map)/np.max(self.visibility_map)
    ...
    def convert_to_unit_interval(self,map_):
        ...
        grid_size = (map_.shape[0], map_.shape[1])
        visibility_map_normalized = np.empty(shape=(grid_size[0], grid_size[1], grid_size[0], grid_size[1]))
        for row in range(grid_size[0]):
            for column in range(grid_size[1]):
                visibility_map_normalized[:, :, row, column] = map_[:, :, row, column] - np.min(map_[:, :, row, column])
                visibility_map_normalized[:, :, row, column] = map_[:, :, row, column] / np.max(map_[:, :, row, column])
    ...
    def at_fixation_fovea(self, fixation):
        " Given a fixation in the grid, it returns the visibility map, represented as a 2D array of scalars "
        """ Input:
                fixation (int, int) : cell in the grid on which the observer is fixating
            Output:
                visibility_map (2D array of floats) : matrix the size of the grid, where each value is a scalar which represents how much the view diminishes
        """
        return self.fovea_map[:, :, fixation[0], fixation[1]]
    
    def normalized_fovea_at_every_fixation(self):
        ...
        return self.fovea_map_normalized
