...
def load_config(config_dir, config_name, image_size, max_scanpath_length, number_of_processes, human_scanpaths,follow_human_scanpath):
    ...
    config['save_probability_maps'] = (follow_human_scanpath) or ('save_probability_maps' in config.keys() and config['save_probability_maps'])

    config['save_similarity_maps'] = config['save_similarity_maps'] and (human_scanpaths == {})
    config['human_scanpaths']       = human_scanpaths
    if not (config["history_size"] is None):
        config["history_size"] = int(config["history_size"])
...
def create_output_folders(save_path, image_name, image_range, human_subject):
